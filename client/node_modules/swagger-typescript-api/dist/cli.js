#!/usr/bin/env node
import { d, e, a, b as b$1, f, g } from './chunk-M2JPSLFC.js';
import { resolve } from 'node:path';
import P from 'lodash';
import T from 'didyoumean';

var c=Symbol("root"),E=Symbol("skip"),q=["version","help"];var F=(e,t)=>{if(e==null||!Array.isArray(e))throw "args should be array";let n=e.slice();switch(t){case"electron":return process.defaultApp?n.slice(2):n.slice(1);case"user":return n;default:return n.slice(2)}};T.threshold=.5;var G=(e,t,n)=>{let a=F(e.args,e.from);return new Promise((s,i)=>{let{command:u,usageOptions:r,error:d}=_(t,a);if(d){i(new Error(d));return}!r.length&&u.name===c&&r.push(u.options.find(p=>p.flags.name==="help"));let l=r.filter(p=>p.operation);if(l.length){l[0].operation(),s({command:E,options:{}});return}else {let p="",g=(m,f)=>{if(m){let h=m.$data;if(!h.length&&!f.flags.value)return !f.flags.isNoFlag;if(f.flags.value)return f.flags.value.variadic?h.reduce((w,y)=>(w.push(...y.split(",").map(f.flags.value.formatter)),w),[]):f.flags.value.formatter(h[0]||f.default)}return f.default},$=u.options.reduce((m,f)=>{if(p)return m;let h=r.find(y=>y.flags.name===f.flags.name);if(!h&&f.required)return p=`required option '${f.flags.raw}' not specified`,m;let w=g(h,f);if(!f.operation){let y=f.internal||{};m[y.name||f.flags.name]=y.formatter?y.formatter(w):w;}return m},{});p?i(new Error(p)):s({command:u.name===c?null:u.name,options:$});}})},_=(e,t)=>{let n=null,a=[],s=null,i="",u=[];return t.forEach((r,d)=>{if(!i){if(d===0){if(n=e[r],!n&&!r.startsWith("-")){let l=T(r,e.keys());i=`unknown command ${r}${l?`
(Did you mean ${l} ?)`:""}`;}else n||(n=e[c]);n&&(u=n.options.reduce((l,p)=>[...l,...p.flags.keys],[]));}if(!i){if(r.startsWith("-")){let l=n.options.find(p=>p.flags.keys.includes(r));if(!l){let p=T(r,u);i=`unknown option ${r}${p?`
(Did you mean ${p} ?)`:""}`;}if(l){if(s&&s.flags.name===l.flags.name)return;let p=a.find(g=>g.flags.name===l.flags.name);p?s=p:(s={...l,$data:[]},a.push(s));}return}s&&s.$data.push(r);}}}),n=n||e[c],i?{command:null,usageOptions:[],error:i}:{command:n,usageOptions:a,error:i}};var A=e=>e.reduce((t,n)=>{let a=`${n.flags.keys.join(", ")}${n.flags.value?.raw?` ${n.flags.value?.raw}`:""}`,s=`${n.description||""}${n.default===void 0||n.flags.isNoFlag&&n.default===!0?"":` (default: ${typeof n.default=="string"?`"${n.default}"`:n.default})`}`;return a.length>t.maxLength&&(t.maxLength=a.length),t.options.push({flags:a,description:s}),t},{options:[],maxLength:0}),C=(e,t,n)=>e.map(a=>{let i=`${Array(n).fill(" ").join("")}${a.flags.padEnd(t," ")}  `,u=Array(i.length).fill(" ").join(""),r=a.description.split(`
`);return i+r.map((d,l)=>l===0?d:`
${u}${d}`).join("")}).join(`
`),D=(e,t)=>{let{options:n,maxLength:a}=A(e[c].options),{commands:s,maxLength:i}=P.filter(e,l=>l.name!==c).reduce((l,p)=>{let g=A(p.options),$=`${p.name}${g.length?" [options]":""}`,m=p.description,f=Math.max($.length,g.maxLength);return f>l.maxLength&&(l.maxLength=f),l.commands.push({description:m,name:$,options:g}),l},{commands:[],maxLength:a}),u=C(n,a,2),r=s.map(l=>{let p=`  ${l.name.padEnd(i," ")}    `,g=Array(p.length).fill(" ").join(""),$=l.description.split(`
...
`);console.log(`Usage: ${[t.input.name,t.input.alias].filter(Boolean).join("|")}${u?" [options]":""}${r?" [command]":""}
${t.input.description&&`
${t.input.description}`}

${d}`);},B=(e,t,n)=>{if(n.name===c)return D(e,t);let{options:a,maxLength:s}=A(n.options),i=C(a,s,2),u=[i&&`Options:
...
${u}`);};var S=e=>{console.log(e.input.version);};var H={number:e=>+e,numeric:e=>+e,str:e=>`${e}`,string:e=>`${e}`,bool:e=>!!e,boolean:e=>!!e},U=e=>{let t=null,n=[],a=null,s=e.includes("--no-"),i=P.compact(e.split(" ").map(r=>r.replace(/,/g,"")));for(let r of i)if(r.startsWith("-"))n.push(r);else if(a===null&&(r.startsWith("{")||r.startsWith("[")||r.startsWith("<"))){let d=r.replace(/[{[<>}\].]/g,""),l=r.includes("...");a={raw:r,variadic:l,name:d,formatter:H[d]||H.string};}let u=n.slice().sort((r,d)=>d.length-r.length)[0];return u!==""&&(t=P.camelCase((s?u.replace("--no-",""):u).replace(/(--?)/,""))),{isNoFlag:s,raw:e,name:t,keys:n,value:a}},x=e=>{let t=U(e.flags);return t.name?{required:!!e.required,description:`${e.description||""}`,default:e.default,flags:t,operation:e.operation,internal:e.internal}:(console.warn("invalid option",e),null)};var I=e=>{let t={},n=(s,{addVersion:i=!1,addHelp:u=!0}={})=>(t[s.name]={name:s.name,description:`${s.description||""}`,options:P.compact(s.options.map(x))},i&&t[s.name].options.unshift(x({flags:"-v, --version",description:"output the current version",operation:()=>S(a)})),u&&t[s.name].options.push(x({flags:"-h, --help",description:"display help for command",operation:()=>B(t,a,t[s.name])})),a),a={commands:t,input:e,addCommand:n,execute:s=>G(s,t)};n({name:c,options:[]},{addVersion:!1,addHelp:!1});for(let s of e.options){let i=x(s);if(!i)return;if(q.includes(i.name)){console.warn("reserved option",i.name);return}t[c].options.push(i);}t[c].options.unshift(x({flags:"-v, --version",description:"output the current version",operation:()=>S(a)})),t[c].options.push(x({flags:"-h, --help",description:"display help for command",operation:()=>B(t,a,t[c])}));for(let s of Object.entries(e.commands??{}))n(s);return a};var o=new d({}),b=new e({}),W=I({name:a.name,alias:"sta",version:a.version,description:`Generate api via swagger scheme.
